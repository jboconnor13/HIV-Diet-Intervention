...
fecal.tax.plot.data <- qiime2R::qza_to_phyloseq(features="Data/QIIME_Files/Fecal/Alpine_Output_070324/sepp-filtered-rarefied.qza",
...
#We also pull the matrices
fecal.wu.matrix <-read.table('Data/QIIME_Files/Fecal/Diversity_Results/WU_Matrix/distance-matrix.tsv', header=TRUE, sep='\t', row.names=1)
fecal.uu.matrix <-read.table('Data/QIIME_Files/Fecal/Diversity_Results/UU_Matrix/distance-matrix.tsv', header=TRUE, sep='\t', row.names=1)

#We also pull the SCNIC Modules 
#We initially read the biom file in
fecal.scnic <- qiime2R::qza_to_phyloseq(features="Data/QIIME_Files/Fecal/SCNIC_modules_Output/collapsed.qza",
                                      metadata="Data/QIIME_Files/Fecal/Metadata/metadata.tsv")
for (v in clin_vars_add) {
  fecal.scnic@sam_data[v] <- NA
  col_v_qiime <- which(colnames(fecal.scnic@sam_data)==v)
  col_v_clin <- which(colnames(clin)==v)
  for (r in 1:nrow(fecal.scnic@sam_data)) {
   stud_r <- fecal.scnic@sam_data$StudyID[r]
    tp_r <- fecal.scnic@sam_data$TimepointNumeric[r]
    clin_row <- which(clin$StudyID==stud_r & clin$TimepointNumeric==tp_r)
    if (length(clin_row)==1) {
     fecal.scnic@sam_data[r,col_v_qiime] <- clin[clin_row,col_v_clin]  
    }
  }
}
...
biopsy.wu.matrix <-read.table('Data/QIIME_Files/Biopsy/Diversity_Results/WU_Matrix/distance-matrix.tsv', header=TRUE, sep='\t', row.names=1)
biopsy.uu.matrix <-read.table('Data/QIIME_Files/Biopsy/Diversity_Results/UU_Matrix/distance-matrix.tsv', header=TRUE, sep='\t', row.names=1)
...
```{r}
#Baseline Data is pulled 
fecal.scnic.t1.data <- subset_samples(fecal.scnic, Timepoint == "T1") 

#For the HIV comparisons we select MSM samples (to compare HIV(+)MSM vs HIV(-)MSM)
fecal.scnic.msm.data <- subset_samples(fecal.scnic.t1.data, MSM_status == "MSM")

#We use ANCOM-BC to assess differences by HIV Status
fecal.scnic.hiv_diff <- ancombc2(
  data = fecal.scnic.msm.data,
  fix_formula = 'HIV_Status',
  p_adj_method="fdr"
)
...
#For the HIV comparisons we select MSM samples (to compare HIV(+)MSM vs HIV(-)MSM)
fecal.scnic.hiv.data <- subset_samples(fecal.scnic.t1.data, HIV_Status == "Negative")

#We use ANCOM-BC to assess differences by HIV Status
fecal.scnic.msm_diff <- ancombc2(
  data = fecal.scnic.hiv.data,
  fix_formula = 'MSM_status',
  p_adj_method="fdr"
)
...
```{r}
#Data from Agrarian Subjects is pulled 
fecal.scnic.ag.data <- subset_samples(fecal.scnic, Diet == "Agrarian") 

#We use ANCOM-BC to assess longitudinal differences assessing for changes using random effect of study ID
fecal.scnic.ag_diff <- ancombc2(
  data = fecal.scnic.ag.data, 
  fix_formula = 'TimepointNumeric',
  p_adj_method="fdr",
  rand_formula = '(1 | StudyID)'
)
...
delta_vars <- c("StudyID","Diet","Group","HIV_Status", "MSM_status")

fecal.delta <- unique(fecal.tax.plot.data@sam_data[,delta_vars])

fecal.delta$WU_T1T2 <- NA
fecal.delta$WU_T1T3 <- NA
fecal.delta$UU_T1T2 <- NA
fecal.delta$UU_T1T3 <- NA
fecal.delta$Group <- factor(fecal.delta$Group, levels=c("HIV(+)MSM","HIV(-)MSM","HIV(-)MSW"))

for (j in 1:nrow(fecal.delta)) {
  stud_j <- fecal.delta$StudyID[j] 
  rowname_1 <- rownames(fecal.tax.plot.data@sam_data)[which(fecal.tax.plot.data@sam_data$StudyID==stud_j & fecal.tax.plot.data@sam_data$TimepointNumeric==1)]
  rowname_2 <- rownames(fecal.tax.plot.data@sam_data)[which(fecal.tax.plot.data@sam_data$StudyID==stud_j & fecal.tax.plot.data@sam_data$TimepointNumeric==2)]
  rowname_3 <- rownames(fecal.tax.plot.data@sam_data)[which(fecal.tax.plot.data@sam_data$StudyID==stud_j & fecal.tax.plot.data@sam_data$TimepointNumeric==3)]
  if(length(rowname_1)==1) {
    uu_row_1 <- which(rownames(fecal.uu.matrix)==rowname_1)
    wu_row_1 <- which(rownames(fecal.wu.matrix)==rowname_1)
    if(length(rowname_2)==1) {
        uu_col_2 <- which(colnames(fecal.uu.matrix)==rowname_2)
        wu_col_2 <- which(colnames(fecal.wu.matrix)==rowname_2)
        uu_12 <- as.numeric(fecal.uu.matrix[uu_row_1,uu_col_2])
        wu_12 <- as.numeric(fecal.wu.matrix[wu_row_1,wu_col_2])
        fecal.delta$UU_T1T2[j] <- uu_12
        fecal.delta$WU_T1T2[j] <- wu_12
    }
    if(length(rowname_3)==1) {
        uu_col_3 <- which(colnames(fecal.uu.matrix)==rowname_3)
        wu_col_3 <- which(colnames(fecal.wu.matrix)==rowname_3)
        uu_13 <- as.numeric(fecal.uu.matrix[uu_row_1,uu_col_3])
        wu_13 <- as.numeric(fecal.wu.matrix[wu_row_1,wu_col_3])
        fecal.delta$UU_T1T3[j] <- uu_13
        fecal.delta$WU_T1T3[j] <- wu_13
    }
  }
}


fecal.delta.filt <- fecal.delta[which(fecal.delta$StudyID %in% clin$StudyID),]
fecal.delta.data.frame <- as.data.frame(fecal.delta.filt@.Data)
colnames(fecal.delta.data.frame) <- names(fecal.delta.filt)

fecal.delta.data.frame$AllVar <- "AllVar"
fecal.group.stats <- gp_stat_sum(data=fecal.delta.data.frame,
                                 sub_vars="AllVar",
                                dep_vars=c("WU_T1T2","WU_T1T3","UU_T1T2","UU_T1T3"),
                                gp_var="Group",
                                normality=TRUE
                                )

fecal.group.stats_diet <- gp_stat_sum(data=fecal.delta.data.frame,
                                 sub_vars="Group",
                                dep_vars=c("WU_T1T2","WU_T1T3","UU_T1T2","UU_T1T3"),
                                gp_var="Diet",
                                normality=TRUE
...
fecal.wu_plot_12 <- ggplot(fecal.delta.data.frame, aes(x=Group, y=WU_T1T2, fill=Diet)) +
  geom_boxplot(outliers = FALSE) +
  geom_jitter()+
  scale_y_log10() +
  scale_fill_manual(values=c("darkgreen","darkorange", "darkgreen","darkorange", "darkgreen","darkorange")) +
  labs(y="WU T1-T2", x="Group", tag="A")+
  theme_bw() +
  theme(
...
fecal.wu_plot_13 <- ggplot(fecal.delta.data.frame, aes(x=Group, y=WU_T1T3, fill=Diet)) +
  geom_boxplot(outliers = FALSE) +
  geom_jitter()+
  scale_y_log10() +
  scale_fill_manual(values=c("darkgreen","darkorange", "darkgreen","darkorange", "darkgreen","darkorange")) +
  labs(y="WU T1-T3", x="Group", tag="B")+
  theme_bw() +
  theme(
    title = element_text(size = 15),
    axis.text.y = element_text(size = 15),
...
fecal.wu.delta <- ggarrange(fecal.wu_plot_12, fecal.wu_plot_13)
...
biopsy.delta <- unique(biopsy.tax.plot.data.filt@sam_data[,delta_vars])

biopsy.delta$WU_T1T2 <- NA
biopsy.delta$WU_T1T3 <- NA
biopsy.delta$UU_T1T2 <- NA
biopsy.delta$UU_T1T3 <- NA
biopsy.delta$Group <- factor(biopsy.delta$Group, levels=c("HIV(+)MSM","HIV(-)MSM","HIV(-)MSW"))

for (j in 1:nrow(biopsy.delta)) {
  stud_j <- biopsy.delta$StudyID[j] 
  rowname_1 <- rownames(biopsy.tax.plot.data.filt@sam_data)[which(biopsy.tax.plot.data.filt@sam_data$StudyID==stud_j & biopsy.tax.plot.data.filt@sam_data$TimepointNumeric==1)]
  rowname_2 <- rownames(biopsy.tax.plot.data.filt@sam_data)[which(biopsy.tax.plot.data.filt@sam_data$StudyID==stud_j & biopsy.tax.plot.data.filt@sam_data$TimepointNumeric==2)]
  rowname_3 <- rownames(biopsy.tax.plot.data.filt@sam_data)[which(biopsy.tax.plot.data.filt@sam_data$StudyID==stud_j & biopsy.tax.plot.data.filt@sam_data$TimepointNumeric==3)]
  if(length(rowname_1)==1) {
    uu_row_1 <- which(rownames(biopsy.uu.matrix)==rowname_1)
    wu_row_1 <- which(rownames(biopsy.wu.matrix)==rowname_1)
    if(length(rowname_2)==1) {
      uu_col_2 <- which(colnames(biopsy.uu.matrix)==rowname_2)
      wu_col_2 <- which(colnames(biopsy.wu.matrix)==rowname_2)
      uu_12 <- as.numeric(biopsy.uu.matrix[uu_row_1,uu_col_2])
      wu_12 <- as.numeric(biopsy.wu.matrix[wu_row_1,wu_col_2])
      biopsy.delta$UU_T1T2[j] <- uu_12
      biopsy.delta$WU_T1T2[j] <- wu_12
    }
    if(length(rowname_3)==1) {
      uu_col_3 <- which(colnames(biopsy.uu.matrix)==rowname_3)
      wu_col_3 <- which(colnames(biopsy.wu.matrix)==rowname_3)
      uu_13 <- as.numeric(biopsy.uu.matrix[uu_row_1,uu_col_3])
      wu_13 <- as.numeric(biopsy.wu.matrix[wu_row_1,wu_col_3])
      biopsy.delta$UU_T1T3[j] <- uu_13
      biopsy.delta$WU_T1T3[j] <- wu_13
    }
  }
}


biopsy.delta.filt <- biopsy.delta[which(biopsy.delta$StudyID %in% clin$StudyID),]
biopsy.delta.data.frame <- as.data.frame(biopsy.delta.filt@.Data)
colnames(biopsy.delta.data.frame) <- names(biopsy.delta.filt)

biopsy.delta.data.frame$AllVar <- "AllVar"
biopsy.group.stats <- gp_stat_sum(data=biopsy.delta.data.frame,
                                 sub_vars="AllVar",
                                 dep_vars=c("WU_T1T2","WU_T1T3","UU_T1T2","UU_T1T3"),
                                 gp_var="Group",
                                 normality=TRUE
...
biopsy.group.stats_diet <- gp_stat_sum(data=biopsy.delta.data.frame,
                                      sub_vars="Group",
                                      dep_vars=c("WU_T1T2","WU_T1T3","UU_T1T2","UU_T1T3"),
                                      gp_var="Diet",
                                      normality=TRUE
...
biopsy.wu_plot_12 <- ggplot(biopsy.delta.data.frame, aes(x=Group, y=WU_T1T2, fill=Diet)) +
  geom_boxplot(outliers = FALSE) +
  geom_jitter()+
  scale_y_log10() +
  scale_fill_manual(values=c("darkgreen","darkorange", "darkgreen","darkorange", "darkgreen","darkorange")) +
...
biopsy.wu_plot_13 <- ggplot(biopsy.delta.data.frame, aes(x=Group, y=WU_T1T3, fill=Diet)) +
  geom_boxplot(outliers = FALSE) +
  geom_jitter()+
  scale_y_log10() +
  scale_fill_manual(values=c("darkgreen","darkorange", "darkgreen","darkorange", "darkgreen","darkorange")) +
...
biopsy.wu.delta <- ggarrange(biopsy.wu_plot_12, biopsy.wu_plot_13)
