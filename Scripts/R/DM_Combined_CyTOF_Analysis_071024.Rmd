...
bd_lmer_data <- bd_all_proc_imp
bd_lmer_data$HIV <- factor(bd_lmer_data$HIV, levels=c("HIV-Negative","HIV-Positive"))
bd_lmer_data$MSM <- gsub("NonMSM","MSW",as.character(bd_lmer_data$MSM))
bd_lmer_data$MSM <- factor(bd_lmer_data$MSM, levels=c("MSW","MSM"))
bd_lmer_data$Diet <- factor(bd_lmer_data$Diet, levels=c("Western","Agrarian"))
bd_lmer_data$Timepoint <- factor(bd_lmer_data$Timepoint, levels=c("T1","T3"))
bd_lmer_data$Age <- as.numeric(bd_lmer_data$Age)

#The patient vars are defined
patient_vars_diet <- c("Age","HIV","MSM","Diet","Timepoint")

#The cells are pulled from the data set 
#First we need to remove the "_Norm" to avoid errors
bd_orig_lmer_cols <- colnames(bd_lmer_data)[18:ncol(bd_lmer_data)]
colnames(bd_lmer_data)[18:ncol(bd_lmer_data)] <- paste0("c_",18:ncol(bd_lmer_data))

bd_cell_vars <- colnames(bd_lmer_data)[18:ncol(bd_lmer_data)]
#I set up a basic diet data set for the linear mixed effects results

#We generate an linear mixed effects model data set
bd_patient_lm_data <- unique(expand.grid(patient_vars_diet, bd_cell_vars))
colnames(bd_patient_lm_data) <- c("Factor","Outcome")
#We generate fields for the statistics
#One for the p-value
bd_patient_lm_data["pval"] <- NA
#One for the coefficient
bd_patient_lm_data["coef"] <- NA
#One for the significance label
bd_patient_lm_data["sig"] <- NA
#One field for displayng the actual cell values
bd_patient_lm_data["DisplayOutcome"] <- NA

for (response in bd_cell_vars){ 
  # create full model 
  ...
  for(predictor in patient_vars_diet){ 
    # create null models 
    pred <- patient_vars_diet[! patient_vars_diet %in% c(predictor)]
    ...
    res <- anova(nm, fm)
    betas <- fm@beta
    cols <- c("Intercept", patient_vars_diet)
    ...
    #We now identify the row in the stat table and add the corresponding p and beta values
      stat_row <- which(bd_patient_lm_data$Factor==predictor & bd_patient_lm_data$Outcome==response)
      bd_patient_lm_data$pval[stat_row] <- res$`Pr(>Chisq)`[2]
      bd_patient_lm_data$coef[stat_row] <- spec_beta
      ...
      bd_patient_lm_data$sig[stat_row] <- sig
      
  #We pull the display name
  #To do so, we first select the index
  response_ind <- which(bd_cell_vars==as.character(response))
  #Now we select the original column name 
  display_name <- gsub("_Norm","",bd_orig_lmer_cols[response_ind])
  bd_patient_lm_data$DisplayOutcome[stat_row] <- display_name     
...
###Now we add new field for adjusted p-values 
bd_patient_lm_data["padj"] <- NA
#We go through the factors and adjust with an fdr correction
for (f in patient_vars_diet) {
  bd_patient_lm_data$padj[which(bd_patient_lm_data$Factor==f)] <- p.adjust(bd_patient_lm_data$pval[which(bd_patient_lm_data$Factor==f)], method="fdr")
}

#We create a new field for the adjusted significance, which is same as original significant base (i.e. Positive/Negative so same first 8 letters)
bd_patient_lm_data["sigadj"] <- substr(bd_patient_lm_data$sig,1,8)

#Now we go through each adjusted p-value and coefficients to generate adjusted significance
for (r in 1:nrow(bd_patient_lm_data)) {
  padj_r <- bd_patient_lm_data$padj[r]
  if (padj_r<0.05) {
    bd_patient_lm_data$sigadj[r] <- paste0(bd_patient_lm_data$sigadj[r]," (Significant)")
  }
}
  
#Now we plot those results 
bd_patient_lm_plot <- ggplot(bd_patient_lm_data, aes(x = Factor, y = DisplayOutcome, color=sigadj)) +
...
bd_sig_cells <- unique(bd_patient_lm_data$DisplayOutcome[which(bd_patient_lm_data$padj<0.05)])

#We pull the data for those cells
bd_sig_cell_data <- bd_patient_lm_data[which(bd_patient_lm_data$DisplayOutcome %in% bd_sig_cells),]

#Now we plot those results
sig_bd_patient_lm_plot <- ggplot(bd_sig_cell_data, aes(x = Factor, y = DisplayOutcome, color=sigadj)) +
  scale_color_manual(values=c("lightblue","blue","pink","red"))+
  geom_point(size=5) +
  theme_minimal() +
  labs(x = "Predictor", y = "Response", fill=" ", ) +
...
ag_bd_lmer_data <- bd_lmer_data %>% 
                  filter(Diet=="Agrarian")

#Now we set variables without diet in them 
patient_vars <- c("Age","HIV","MSM","Timepoint")
# build models and test for significance 
#We generate a new data set for the linear mixed effects modelling including the 

#We generate an linear mixed effects model data set
bd_patient_ag_lm_data <- unique(expand.grid(patient_vars, bd_cell_vars))
colnames(bd_patient_ag_lm_data) <- c("Factor","Outcome")
#We generate fields for the statistics
#One for the p-value
bd_patient_ag_lm_data["pval"] <- NA
#One for the coefficient
bd_patient_ag_lm_data["coef"] <- NA
#One for the significance label
bd_patient_ag_lm_data["sig"] <- NA
#One field for displayng the actual cell values
bd_patient_ag_lm_data["DisplayOutcome"] <- NA

for (response in bd_cell_vars){ 
  # create full model 
  ff <- as.formula(paste0(response," ~ Age + HIV + MSM + Timepoint + (1|StudyID)")) 
  fm <- lmer(ff,
            data = ag_bd_lmer_data,
            REML = F)
  ...
  for(predictor in patient_vars){ 
    ...
    #We now identify the row in the stat table and add the corresponding p and beta values
      stat_row <- which(bd_patient_ag_lm_data$Factor==predictor & bd_patient_ag_lm_data$Outcome==response)
      bd_patient_ag_lm_data$pval[stat_row] <- res$`Pr(>Chisq)`[2]
      bd_patient_ag_lm_data$coef[stat_row] <- spec_beta
      #If beta is positive the directionality is positive
      if (spec_beta>0) {
        sig <- "Positive"
      #If beta is negative the directionality is negative
      ...
      bd_patient_ag_lm_data$sig[stat_row] <- sig
  ...
  #Now we select the original column name 
  display_name <- gsub("_Norm","",bd_orig_lmer_cols[response_ind])
  bd_patient_ag_lm_data$DisplayOutcome[stat_row] <- display_name     
...
###Now we add new field for adjusted p-values 
bd_patient_ag_lm_data["padj"] <- NA
#We go through the factors and adjust with an fdr correction
for (f in patient_vars) {
  bd_patient_ag_lm_data$padj[which(bd_patient_ag_lm_data$Factor==f)] <- p.adjust(bd_patient_ag_lm_data$pval[which(bd_patient_ag_lm_data$Factor==f)], method="fdr")
}

#We create a new field for the adjusted significance, which is same as original significant base (i.e. Positive/Negative so same first 8 letters)
bd_patient_ag_lm_data["sigadj"] <- substr(bd_patient_ag_lm_data$sig,1,8)

#Now we go through each adjusted p-value and coefficients to generate adjusted significance
for (r in 1:nrow(bd_patient_ag_lm_data)) {
  padj_r <- bd_patient_ag_lm_data$padj[r]
  #Now if the adjusted p-value is below 0.05 we add significcme
  if (padj_r<0.05) {
    bd_patient_ag_lm_data$sigadj[r] <- paste0(bd_patient_ag_lm_data$sigadj[r]," (Significant)")
  }
}

#Now we plot those results 
bd_patient_ag_lm_plot <- ggplot(bd_patient_ag_lm_data, aes(x = Factor, y = DisplayOutcome, color=sigadj)) +
  scale_color_manual(values=c("lightblue","blue","pink","red"))+
  geom_point(size=5) +
  theme_minimal() +
  labs(x = "Predictor", y = "Response", fill=" ", ) +
...
#For simplicity we will focus on the significant variables 
bd_sig_ag_cells <- unique(bd_patient_ag_lm_data$DisplayOutcome[which(bd_patient_ag_lm_data$padj<0.05)])

#We pull the data for those cells
bd_sig_ag_cell_data <- bd_patient_ag_lm_data[which(bd_patient_ag_lm_data$DisplayOutcome %in% bd_sig_ag_cells),]

#We also adjust the display outcomes to include T cells if needed
bd_sig_ag_cell_data <- bd_sig_ag_cell_data %>%
  mutate(DisplayOutcomeFull = ifelse(
    substr(DisplayOutcome, 1, 3) %in% c("CD8", "CD4") & !grepl("T cell", DisplayOutcome, ignore.case = TRUE) & !grepl("Tregs", DisplayOutcome, ignore.case = TRUE),
    paste0(DisplayOutcome, " T Cells"),
    DisplayOutcome
  ))

#Now we remove % of parent
bd_sig_ag_cell_data$DisplayOutcomeFull <- gsub("\\(% of parent\\)","",bd_sig_ag_cell_data$DisplayOutcomeFull)
#We also add CD in front of CD16+
bd_sig_ag_cell_data$DisplayOutcomeFull <- gsub("\\+16","+CD16",bd_sig_ag_cell_data$DisplayOutcomeFull)
#Now we plot those results
sig_bd_patient_ag_lm_plot <- ggplot(bd_sig_ag_cell_data, aes(x = Factor, y = DisplayOutcomeFull, color=sigadj)) +
  scale_color_manual(values=c("lightblue","blue","pink","red"))+
  geom_point(size=5) +
  theme_minimal() +
  labs(x = "Predictor", y = "Response", fill=" ", tag="A") +
  ggtitle("Agrarian Diet") +
  theme_classic() +
  guides(color = guide_legend(ncol = 2))+
  theme(legend.position = "bottom",
        panel.grid.major = element_line(color = "gray", linetype = "solid"),
...
we_bd_lmer_data <- bd_lmer_data %>% 
                  filter(Diet=="Western")


#We generate an linear mixed effects model data set
bd_patient_we_lm_data <- unique(expand.grid(patient_vars, bd_cell_vars))
colnames(bd_patient_we_lm_data) <- c("Factor","Outcome")
#We generate fields for the statistics
#One for the p-value
bd_patient_we_lm_data["pval"] <- NA
#One for the coefficient
bd_patient_we_lm_data["coef"] <- NA
#One for the significance label
bd_patient_we_lm_data["sig"] <- NA
#One field for displayng the actual cell values
bd_patient_we_lm_data["DisplayOutcome"] <- NA

for (response in bd_cell_vars){ 
  # create full model 
  ff <- as.formula(paste0(response," ~ Age + HIV + MSM + Timepoint + (1|StudyID)")) 
  fm <- lmer(ff,
            data = we_bd_lmer_data,
            REML = F)
  ...
  for(predictor in patient_vars){ 
    ...
    #We now identify the row in the stat table and add the corresponding p and beta values
      stat_row <- which(bd_patient_we_lm_data$Factor==predictor & bd_patient_we_lm_data$Outcome==response)
      bd_patient_we_lm_data$pval[stat_row] <- res$`Pr(>Chisq)`[2]
      bd_patient_we_lm_data$coef[stat_row] <- spec_beta
      #If beta is positive the directionality is positive
      if (spec_beta>0) {
        sig <- "Positive"
      #If beta is negative the directionality is negative
      ...
      bd_patient_we_lm_data$sig[stat_row] <- sig
  ...
  #Now we select the original column name 
  display_name <- gsub("_Norm","",bd_orig_lmer_cols[response_ind])
  bd_patient_we_lm_data$DisplayOutcome[stat_row] <- display_name     
...
###Now we add new field for adjusted p-values 
bd_patient_we_lm_data["padj"] <- NA
#We go through the factors and adjust with an fdr correction
for (f in patient_vars) {
  bd_patient_we_lm_data$padj[which(bd_patient_we_lm_data$Factor==f)] <- p.adjust(bd_patient_we_lm_data$pval[which(bd_patient_we_lm_data$Factor==f)], method="fdr")
}

#We create a new field for the adjusted significance, which is same as original significant base (i.e. Positive/Negative so same first 8 letters)
bd_patient_we_lm_data["sigadj"] <- substr(bd_patient_we_lm_data$sig,1,8)

#Now we go through each adjusted p-value and coefficients to generate adjusted significance
for (r in 1:nrow(bd_patient_we_lm_data)) {
  padj_r <- bd_patient_we_lm_data$padj[r]
  #Now if the adjusted p-value is below 0.05 we add significcme
  if (padj_r<0.05) {
    bd_patient_we_lm_data$sigadj[r] <- paste0(bd_patient_we_lm_data$sigadj[r]," (Significant)")
  }
}

#Now we plot those results 
bd_patient_we_lm_plot <- ggplot(bd_patient_we_lm_data, aes(x = Factor, y = DisplayOutcome, color=sigadj)) +
  scale_color_manual(values=c("lightblue","blue","pink","red"))+
  geom_point(size=5) +
  theme_minimal() +
  labs(x = "Predictor", y = "Response", fill=" ", ) +
...
#We pull the data for those cells that were significantly different in the agrarian diet in some way
sig_we_cell_data <- bd_patient_we_lm_data[which(bd_patient_we_lm_data$DisplayOutcome %in% bd_sig_ag_cell_data$DisplayOutcome),]

#We also adjust the display outcomes to include T cells if needed
sig_we_cell_data <- sig_we_cell_data %>%
  mutate(DisplayOutcomeFull = ifelse(
    substr(DisplayOutcome, 1, 3) %in% c("CD8", "CD4") & !grepl("T cell", DisplayOutcome, ignore.case = TRUE) & !grepl("Tregs", DisplayOutcome, ignore.case = TRUE),
    paste0(DisplayOutcome, " T Cells"),
    DisplayOutcome
...
#Now we remove % of parent
sig_we_cell_data$DisplayOutcomeFull <- gsub("\\(% of parent\\)","",sig_we_cell_data$DisplayOutcomeFull)
#We now correct the 16+ to be CD16+ 
sig_we_cell_data$DisplayOutcomeFull <- gsub("\\+16","+CD16",sig_we_cell_data$DisplayOutcomeFull)

#Now we plot those results
sig_bd_patient_we_lm_plot <- ggplot(sig_we_cell_data, aes(x = Factor, y = DisplayOutcomeFull, color=sigadj)) +
...
#Now we combine plots
supp_8 <- ggarrange(sig_bd_patient_ag_lm_plot,sig_bd_patient_we_lm_plot, common.legend = TRUE, legend="bottom")
```


#####Hidden Confounders

Now we will take those cell types that changed and see how they relate to Metabolic and Inflammtory Markers 
...
#We will focus on cells that changed with time on the agrarian data

ag_tm_bd_cell_vars <-unique(bd_patient_ag_lm_data$Outcome[which(bd_patient_ag_lm_data$padj<0.05 & bd_patient_ag_lm_data$Factor=="Timepoint")])

#We generate an linear mixed effects model data set
bd_hc_lm_data <- unique(expand.grid(hc_vars, ag_tm_bd_cell_vars))
colnames(bd_hc_lm_data) <- c("Factor","Outcome")
#We generate fields for the statistics
#One for the p-value
bd_hc_lm_data["pval"] <- NA
#One for the coefficient
bd_hc_lm_data["coef"] <- NA
#One for the significance label
bd_hc_lm_data["sig"] <- NA
#One field for displayng the actual cell values
bd_hc_lm_data["DisplayOutcome"] <- NA

bd_lmer_data$Age <- as.numeric(bd_lmer_data$Age)
bd_lmer_data$HDLNorm <- as.numeric(bd_lmer_data$HDLNorm)
bd_lmer_data$LDLNorm <- as.numeric(bd_lmer_data$LDLNorm)
bd_lmer_data$TriglyceridesNorm <- as.numeric(bd_lmer_data$TriglyceridesNorm)
bd_lmer_data$IL6Norm <- as.numeric(bd_lmer_data$IL6Norm)
bd_lmer_data$CRPNorm <- as.numeric(bd_lmer_data$CRPNorm)
bd_lmer_data$LBPNorm <- as.numeric(bd_lmer_data$LBPNorm)


for (response in ag_tm_bd_cell_vars){ 
  # create full model 
  ...
  for(predictor in hc_vars){ 
    ...
    #We now identify the row in the stat table and add the corresponding p and beta values
      stat_row <- which(bd_hc_lm_data$Factor==predictor & bd_hc_lm_data$Outcome==response)
      bd_hc_lm_data$pval[stat_row] <- res$`Pr(>Chisq)`[2]
      bd_hc_lm_data$coef[stat_row] <- spec_beta
      #If beta is positive the directionality is positive
      if (spec_beta>0) {
        sig <- "Positive"
      #If beta is negative the directionality is negative
      ...
      bd_hc_lm_data$sig[stat_row] <- sig
    #We pull the display name
  #To do so, we first select the index
  response_ind <- which(bd_cell_vars==as.character(response))
  #Now we select the original column name 
  display_name <- gsub("_Norm","",bd_orig_lmer_cols[response_ind])
  bd_hc_lm_data$DisplayOutcome[stat_row] <- display_name     
...
###Now we add new field for adjusted p-values 
bd_hc_lm_data["padj"] <- NA
#We go through the factors and adjust with an fdr correction
for (f in hc_vars) {
  bd_hc_lm_data$padj[which(bd_hc_lm_data$Factor==f)] <- p.adjust(bd_hc_lm_data$pval[which(bd_hc_lm_data$Factor==f)], method="fdr")
}

#We create a new field for the adjusted significance, which is same as original significant base (i.e. Positive/Negative so same first 8 letters)
bd_hc_lm_data["sigadj"] <- substr(bd_hc_lm_data$sig,1,8)

#Now we go through each adjusted p-value and coefficients to generate adjusted significance
for (r in 1:nrow(bd_hc_lm_data)) {
  padj_r <- bd_hc_lm_data$padj[r]
  #Now if the adjusted p-value is below 0.05 we add significcme
  if (padj_r<0.05) {
    bd_hc_lm_data$sigadj[r] <- paste0(bd_hc_lm_data$sigadj[r]," (Significant)")
  }
}

#Now we plot those results 
bd_hc_lm_plot <- ggplot(bd_hc_lm_data, aes(x = Factor, y = DisplayOutcome, color=sigadj)) +
...
###Data Prepration
#We will be pulling the names of the cells that changed on the diet
bd_sig_ag <- paste0(bd_patient_ag_lm_data$DisplayOutcome[which(bd_patient_ag_lm_data$Factor=="Timepoint" & bd_patient_ag_lm_data$padj<0.05)], "_Norm")

#Now we will select that data for the violin plots
select_bd_all_proc_imp <- bd_all_proc_imp[,c(clinvars, bd_sig_ag)]

#Now we pivot to long format for the values 
select_bd_long <- pivot_longer(select_bd_all_proc_imp, cols=bd_sig_ag, names_to="Measure", values_to="Amount")

#Now we add the DisplayOutcome field 
select_bd_long$DisplayOutcome <- gsub("_Norm","",select_bd_long$Measure)

#Now we add the DiplayOutcomeFull Field 
select_bd_long <- select_bd_long %>%
  mutate(DisplayOutcomeFull = ifelse(
    substr(DisplayOutcome, 1, 3) %in% c("CD8", "CD4") & !grepl("T cell", DisplayOutcome, ignore.case = TRUE) & !grepl("Tregs", DisplayOutcome, ignore.case = TRUE),
    paste0(DisplayOutcome, " T Cells"),
    DisplayOutcome
...
#We add a cetgroy for plotting
select_bd_long <- select_bd_long %>%
  mutate(Category = ifelse(
    substr(DisplayOutcome, 1, 3) %in% c("CD8", "CD4"),
    "T Cells",
    "Monocytes"
  ))

#Below we make some aesthetic changes for plotting
#Now we remove % of parent
select_bd_long$DisplayOutcomeFull <- gsub("\\(% of parent\\)","",select_bd_long$DisplayOutcomeFull)
#We now correct the 16+ to be CD16+ 
select_bd_long$DisplayOutcomeFull <- gsub("\\+16","+CD16",select_bd_long$DisplayOutcomeFull)
##We now provide line dividers as well
select_bd_long$DisplayOutcomeFull <- gsub("\\(%","\n\\(%",select_bd_long$DisplayOutcomeFull)
select_bd_long$DisplayOutcomeFull <- gsub("l Mono","l \nMono",select_bd_long$DisplayOutcomeFull)
select_bd_long$DisplayOutcomeFull <- gsub("e Mono","e \nMono",select_bd_long$DisplayOutcomeFull)


###Statistics Preparation
#We add fields to the original statistics for ggplot to recognize

bd_patient_ag_lm_data["Diet"] <- "Agrarian"
bd_patient_ag_lm_data["group1"] <- "T1"
bd_patient_ag_lm_data["group2"] <- "T3"

bd_patient_we_lm_data["Diet"] <- "Western"
bd_patient_we_lm_data["group1"] <- "T1"
bd_patient_we_lm_data["group2"] <- "T3"

#We not stack the two on top of each other
bd_all_stats <- rbind(bd_patient_we_lm_data,bd_patient_ag_lm_data)

#Now we focus on the stats of interest
bd_all_sig_stats <- bd_all_stats[which(paste0(bd_all_stats$DisplayOutcome,"_Norm") %in% bd_sig_ag & bd_all_stats$Factor=="Timepoint"),]

bd_all_sig_stats <- bd_all_sig_stats %>%
  mutate(DisplayOutcomeFull = ifelse(
    substr(DisplayOutcome, 1, 3) %in% c("CD8", "CD4") & !grepl("T cell", DisplayOutcome, ignore.case = TRUE) & !grepl("Tregs", DisplayOutcome, ignore.case = TRUE),
    paste0(DisplayOutcome, " T Cells"),
    DisplayOutcome
...
#Below we maker aesthetic changes
#Now we remove % of parent
bd_all_sig_stats$DisplayOutcomeFull <- gsub("\\(% of parent\\)","",bd_all_sig_stats$DisplayOutcomeFull)
#We now correct the 16+ to be CD16+ 
bd_all_sig_stats$DisplayOutcomeFull <- gsub("\\+16","+CD16",bd_all_sig_stats$DisplayOutcomeFull)
##We now provide line dividers as well
bd_all_sig_stats$DisplayOutcomeFull <- gsub("\\(%","\n\\(%",bd_all_sig_stats$DisplayOutcomeFull)
bd_all_sig_stats$DisplayOutcomeFull <- gsub("l Mono","l \nMono",bd_all_sig_stats$DisplayOutcomeFull)
bd_all_sig_stats$DisplayOutcomeFull <- gsub("e Mono","e \nMono",bd_all_sig_stats$DisplayOutcomeFull)

#Now we add asterisks 
bd_all_sig_stats$p.signif <- add_asterisks(bd_all_sig_stats$padj)
  
bd_select_viol <- ggplot(data = select_bd_long,
              ...
              theme(legend.position = "bottom",
                ...
                facet_nested(cols=vars(Category,DisplayOutcomeFull),
                  rows=vars(Diet), 
                  scales="free_y")+
                stat_pvalue_manual(
                  bd_all_sig_stats,
...
by_lmer_data <- by_all_proc_imp
by_lmer_data$HIV <- factor(by_lmer_data$HIV, levels=c("HIV-Negative","HIV-Positive"))
by_lmer_data$MSM <- gsub("NonMSM","MSW",as.character(by_lmer_data$MSM))
by_lmer_data$MSM <- factor(by_lmer_data$MSM, levels=c("MSW","MSM"))
by_lmer_data$Diet <- factor(by_lmer_data$Diet, levels=c("Western","Agrarian"))
by_lmer_data$Timepoint <- factor(by_lmer_data$Timepoint, levels=c("T1","T3"))
by_lmer_data$Age <- as.numeric(by_lmer_data$Age)
...
#The cells are pulled from the data set 
#First we need to remove the "_Norm" to avoid errors
by_orig_lmer_cols <- colnames(by_lmer_data)[18:ncol(by_lmer_data)]
colnames(by_lmer_data)[18:ncol(by_lmer_data)] <- paste0("c_",18:ncol(by_lmer_data))

by_cell_vars <- colnames(by_lmer_data)[18:ncol(by_lmer_data)]
#I set up a basic diet data set for the linear mixed effects results

#We generate an linear mixed effects model data set
by_patient_lm_data <- unique(expand.grid(patient_vars_diet, by_cell_vars))
colnames(by_patient_lm_data) <- c("Factor","Outcome")
#We generate fields for the statistics
#One for the p-value
by_patient_lm_data["pval"] <- NA
#One for the coefficient
by_patient_lm_data["coef"] <- NA
#One for the significance label
by_patient_lm_data["sig"] <- NA
#One field for displayng the actual cell values
by_patient_lm_data["DisplayOutcome"] <- NA

for (response in by_cell_vars){ 
  # create full model 
  ff <- as.formula(paste0(response," ~ Age + HIV + MSM + Diet + Timepoint + (1|StudyID)")) 
  fm <- lmer(ff,
             data = by_lmer_data,
  ...
  for(predictor in patient_vars_diet){ 
    ...
    #We now identify the row in the stat table and add the corresponding p and beta values
    stat_row <- which(by_patient_lm_data$Factor==predictor & by_patient_lm_data$Outcome==response)
    by_patient_lm_data$pval[stat_row] <- res$`Pr(>Chisq)`[2]
    by_patient_lm_data$coef[stat_row] <- spec_beta
    #If beta is positive the directionality is positive
    if (spec_beta>0) {
      sig <- "Positive"
      #If beta is negative the directionality is negative
    ...
    by_patient_lm_data$sig[stat_row] <- sig
    
    #We pull the display name
    #To do so, we first select the index
    response_ind <- which(by_cell_vars==as.character(response))
    #Now we select the original column name 
    display_name <- gsub("_Norm","",by_orig_lmer_cols[response_ind])
    by_patient_lm_data$DisplayOutcome[stat_row] <- display_name     
...
###Now we add new field for adjusted p-values 
by_patient_lm_data["padj"] <- NA
#We go through the factors and adjust with an fdr correction
for (f in patient_vars_diet) {
  by_patient_lm_data$padj[which(by_patient_lm_data$Factor==f)] <- p.adjust(by_patient_lm_data$pval[which(by_patient_lm_data$Factor==f)], method="fdr")
}

#We create a new field for the adjusted significance, which is same as original significant base (i.e. Positive/Negative so same first 8 letters)
by_patient_lm_data["sigadj"] <- substr(by_patient_lm_data$sig,1,8)

#Now we go through each adjusted p-value and coefficients to generate adjusted significance
for (r in 1:nrow(by_patient_lm_data)) {
  padj_r <- by_patient_lm_data$padj[r]
  if (padj_r<0.05) {
    by_patient_lm_data$sigadj[r] <- paste0(by_patient_lm_data$sigadj[r]," (Significant)")
  }
}

#Now we plot those results 
by_patient_lm_plot <- ggplot(by_patient_lm_data, aes(x = Factor, y = DisplayOutcome, color=sigadj)) +
  scale_color_manual(values=c("lightblue","blue","pink","red"))+
  geom_point(size=5) +
  theme_minimal() +
  labs(x = "Predictor", y = "Response", fill=" ", ) +
...
#For simplicity we will focus on the significant variables 
by_sig_cells <- unique(by_patient_lm_data$DisplayOutcome[which(by_patient_lm_data$padj<0.05)])

#We pull the data for those cells
by_sig_cell_data <- by_patient_lm_data[which(by_patient_lm_data$DisplayOutcome %in% by_sig_cells),]

#Now we plot those results
sig_by_patient_lm_plot <- ggplot(by_sig_cell_data, aes(x = Factor, y = DisplayOutcome, color=sigadj)) +
  scale_color_manual(values=c("lightblue","blue","pink","red"))+
  geom_point(size=5) +
  theme_minimal() +
  labs(x = "Predictor", y = "Response", fill=" ", ) +
...
#First we focus on Western diet
we_by_lmer_data <- by_lmer_data %>% 
  filter(Diet=="Western")


#We generate an linear mixed effects model data set
by_patient_we_lm_data <- unique(expand.grid(patient_vars, by_cell_vars))
colnames(by_patient_we_lm_data) <- c("Factor","Outcome")
#We generate fields for the statistics
#One for the p-value
by_patient_we_lm_data["pval"] <- NA
#One for the coefficient
by_patient_we_lm_data["coef"] <- NA
#One for the significance label
by_patient_we_lm_data["sig"] <- NA
#One field for displayng the actual cell values
by_patient_we_lm_data["DisplayOutcome"] <- NA

for (response in by_cell_vars){ 
  # create full model 
  ff <- as.formula(paste0(response," ~ Age + HIV + MSM + Timepoint + (1|StudyID)")) 
  fm <- lmer(ff,
             data = we_by_lmer_data,
             REML = F)
  ...
  for(predictor in patient_vars){ 
    ...
    #We now identify the row in the stat table and add the corresponding p and beta values
    stat_row <- which(by_patient_we_lm_data$Factor==predictor & by_patient_we_lm_data$Outcome==response)
    by_patient_we_lm_data$pval[stat_row] <- res$`Pr(>Chisq)`[2]
    by_patient_we_lm_data$coef[stat_row] <- spec_beta
    #If beta is positive the directionality is positive
    if (spec_beta>0) {
      sig <- "Positive"
      #If beta is negative the directionality is negative
    ...
    by_patient_we_lm_data$sig[stat_row] <- sig
    ...
    #Now we select the original column name 
    display_name <- gsub("_Norm","",by_orig_lmer_cols[response_ind])
    by_patient_we_lm_data$DisplayOutcome[stat_row] <- display_name     
...
###Now we add new field for adjusted p-values 
by_patient_we_lm_data["padj"] <- NA
#We go through the factors and adjust with an fdr correction
for (f in patient_vars) {
  by_patient_we_lm_data$padj[which(by_patient_we_lm_data$Factor==f)] <- p.adjust(by_patient_we_lm_data$pval[which(by_patient_we_lm_data$Factor==f)], method="fdr")
}

#We create a new field for the adjusted significance, which is same as original significant base (i.e. Positive/Negative so same first 8 letters)
by_patient_we_lm_data["sigadj"] <- substr(by_patient_we_lm_data$sig,1,8)

#Now we go through each adjusted p-value and coefficients to generate adjusted significance
for (r in 1:nrow(by_patient_we_lm_data)) {
  padj_r <- by_patient_we_lm_data$padj[r]
  #Now if the adjusted p-value is below 0.05 we add significcme
  if (padj_r<0.05) {
    by_patient_we_lm_data$sigadj[r] <- paste0(by_patient_we_lm_data$sigadj[r]," (Significant)")
  }
}

#Now we plot those results 
by_patient_we_lm_plot <- ggplot(by_patient_we_lm_data, aes(x = Factor, y = DisplayOutcome, color=sigadj)) +
  scale_color_manual(values=c("lightblue","blue","pink","red"))+
  geom_point(size=5) +
  theme_minimal() +
  labs(x = "Predictor", y = "Response", fill=" ", tag="B" ) +
...
#We pull the data for those cells that were significantly different in the western diet in some way

#For simplicity we will focus on the significant variables 
by_sig_we_cells <- unique(by_patient_we_lm_data$DisplayOutcome[which(by_patient_we_lm_data$padj<0.05)])

#We pull the data for those cells
by_sig_we_cell_data <- by_patient_we_lm_data[which(by_patient_we_lm_data$DisplayOutcome %in% by_sig_we_cells),]

#We also adjust the display outcomes to include T cells if needed
by_sig_we_cell_data <- by_sig_we_cell_data %>%
  mutate(DisplayOutcomeFull = ifelse(
    substr(DisplayOutcome, 1, 3) %in% c("CD8", "CD4") & !grepl("T cell", DisplayOutcome, ignore.case = TRUE) & !grepl("Tregs", DisplayOutcome, ignore.case = TRUE),
    paste0(DisplayOutcome, " T Cells"),
    DisplayOutcome
...
#Now we remove % of parent
by_sig_we_cell_data$DisplayOutcomeFull <- gsub("\\(% of parent\\)","",by_sig_we_cell_data$DisplayOutcomeFull)
#We now correct the 16+ to be CD16+ 
by_sig_we_cell_data$DisplayOutcomeFull <- gsub("\\+16","+CD16",by_sig_we_cell_data$DisplayOutcomeFull)




#Now we plot those results
sig_by_patient_we_lm_plot <- ggplot(by_sig_we_cell_data, aes(x = Factor, y = DisplayOutcomeFull, color=sigadj)) +
  scale_color_manual(values=c("darkblue","lightblue","pink","red"))+
  geom_point(size=5) +
  theme_minimal() +
  labs(x = "Predictor", y = "Response", fill=" ", tag="B") +
  ggtitle("Western Diet") +
...
#We generate an linear mixed effects model data set
by_patient_ag_lm_data <- unique(expand.grid(patient_vars, by_cell_vars))
colnames(by_patient_ag_lm_data) <- c("Factor","Outcome")
#We generate fields for the statistics
#One for the p-value
by_patient_ag_lm_data["pval"] <- NA
#One for the coefficient
by_patient_ag_lm_data["coef"] <- NA
#One for the significance label
by_patient_ag_lm_data["sig"] <- NA
#One field for displayng the actual cell values
by_patient_ag_lm_data["DisplayOutcome"] <- NA

for (response in by_cell_vars){ 
  # create full model 
  ff <- as.formula(paste0(response," ~ Age + HIV + MSM + Timepoint + (1|StudyID)")) 
  fm <- lmer(ff,
             data = ag_by_lmer_data,
             REML = F)
  ...
  for(predictor in patient_vars){ 
    ...
    #We now identify the row in the stat table and add the corresponding p and beta values
    stat_row <- which(by_patient_ag_lm_data$Factor==predictor & by_patient_ag_lm_data$Outcome==response)
    by_patient_ag_lm_data$pval[stat_row] <- res$`Pr(>Chisq)`[2]
    by_patient_ag_lm_data$coef[stat_row] <- spec_beta
    #If beta is positive the directionality is positive
    if (spec_beta>0) {
      sig <- "Positive"
      #If beta is negative the directionality is negative
    ...
    by_patient_ag_lm_data$sig[stat_row] <- sig
    ...
    #Now we select the original column name 
    display_name <- gsub("_Norm","",by_orig_lmer_cols[response_ind])
    by_patient_ag_lm_data$DisplayOutcome[stat_row] <- display_name     
...
###Now we add new field for adjusted p-values 
by_patient_ag_lm_data["padj"] <- NA
#We go through the factors and adjust with an fdr correction
for (f in patient_vars) {
  by_patient_ag_lm_data$padj[which(by_patient_ag_lm_data$Factor==f)] <- p.adjust(by_patient_ag_lm_data$pval[which(by_patient_ag_lm_data$Factor==f)], method="fdr")
}

#We create a new field for the adjusted significance, which is same as original significant base (i.e. Positive/Negative so same first 8 letters)
by_patient_ag_lm_data["sigadj"] <- substr(by_patient_ag_lm_data$sig,1,8)

#Now we go through each adjusted p-value and coefficients to generate adjusted significance
for (r in 1:nrow(by_patient_ag_lm_data)) {
  padj_r <- by_patient_ag_lm_data$padj[r]
  #Now if the adjusted p-value is below 0.05 we add significcme
  if (padj_r<0.05) {
    by_patient_ag_lm_data$sigadj[r] <- paste0(by_patient_ag_lm_data$sigadj[r]," (Significant)")
  }
}

#Now we plot those results 
by_patient_ag_lm_plot <- ggplot(by_patient_ag_lm_data, aes(x = Factor, y = DisplayOutcome, color=sigadj)) +
  scale_color_manual(values=c("lightblue","blue","pink","red"))+
  geom_point(size=5) +
  theme_minimal() +
  labs(x = "Predictor", y = "Response", fill=" ", tag="A") +
...
#We pull the data for those cells
by_sig_ag_cell_data <- by_patient_ag_lm_data[which(by_patient_ag_lm_data$DisplayOutcome %in% by_sig_we_cells),]


#We also adjust the display outcomes to include T cells if needed
by_sig_ag_cell_data <- by_sig_ag_cell_data %>%
  mutate(DisplayOutcomeFull = ifelse(
    substr(DisplayOutcome, 1, 3) %in% c("CD8", "CD4") & !grepl("T cell", DisplayOutcome, ignore.case = TRUE) & !grepl("Tregs", DisplayOutcome, ignore.case = TRUE),
    paste0(DisplayOutcome, " T Cells"),
    DisplayOutcome
...
#Now we remove % of parent
by_sig_ag_cell_data$DisplayOutcomeFull <- gsub("\\(% of parent\\)","",by_sig_ag_cell_data$DisplayOutcomeFull)
#We also add CD in front of CD16+
by_sig_ag_cell_data$DisplayOutcomeFull <- gsub("\\+16","+CD16",by_sig_ag_cell_data$DisplayOutcomeFull)
#Now we plot those results
sig_by_patient_ag_lm_plot <- ggplot(by_sig_ag_cell_data, aes(x = Factor, y = DisplayOutcomeFull, color=sigadj)) +
  scale_color_manual(values=c("lightblue","blue","pink","red"))+
  geom_point(size=5) +
  theme_minimal() +
  labs(x = "Predictor", y = "Response", fill=" ", tag="A") +
...
#Now we combine plots
supp_8 <- ggarrange(sig_by_patient_ag_lm_plot,sig_by_patient_we_lm_plot, common.legend = TRUE, legend="bottom")
```
...
#We will focus on cells that changed with time on the western data

we_tm_by_cell_vars <-unique(by_patient_we_lm_data$Outcome[which(by_patient_we_lm_data$padj<0.05 & by_patient_we_lm_data$Factor=="Timepoint")])

#We generate an linear mixed effects model data set
by_hc_lm_data <- unique(expand.grid(hc_vars, by_cell_vars))
colnames(by_hc_lm_data) <- c("Factor","Outcome")
#We generate fields for the statistics
#One for the p-value
by_hc_lm_data["pval"] <- NA
#One for the coefficient
by_hc_lm_data["coef"] <- NA
#One for the significance label
by_hc_lm_data["sig"] <- NA
#One field for displayng the actual cell values
by_hc_lm_data["DisplayOutcome"] <- NA

by_lmer_data$Age <- as.numeric(by_lmer_data$Age)
by_lmer_data$HDLNorm <- as.numeric(by_lmer_data$HDLNorm)
by_lmer_data$LDLNorm <- as.numeric(by_lmer_data$LDLNorm)
by_lmer_data$TriglyceridesNorm <- as.numeric(by_lmer_data$TriglyceridesNorm)
by_lmer_data$IL6Norm <- as.numeric(by_lmer_data$IL6Norm)
by_lmer_data$CRPNorm <- as.numeric(by_lmer_data$CRPNorm)
by_lmer_data$LBPNorm <- as.numeric(by_lmer_data$LBPNorm)
...
for (response in by_cell_vars){ 
  ...
  for(predictor in hc_vars){ 
    ...
    #We now identify the row in the stat table and add the corresponding p and beta values
    stat_row <- which(by_hc_lm_data$Factor==predictor & by_hc_lm_data$Outcome==response)
    by_hc_lm_data$pval[stat_row] <- res$`Pr(>Chisq)`[2]
    by_hc_lm_data$coef[stat_row] <- spec_beta
    #If beta is positive the directionality is positive
    if (spec_beta>0) {
      sig <- "Positive"
      #If beta is negative the directionality is negative
    ...
    by_hc_lm_data$sig[stat_row] <- sig
    #We pull the display name
    #To do so, we first select the index
    response_ind <- which(by_cell_vars==as.character(response))
    #Now we select the original column name 
    display_name <- gsub("_Norm","",by_orig_lmer_cols[response_ind])
    by_hc_lm_data$DisplayOutcome[stat_row] <- display_name     
...
###Now we add new field for adjusted p-values 
by_hc_lm_data["padj"] <- NA
#We go through the factors and adjust with an fdr correction
for (f in hc_vars) {
  by_hc_lm_data$padj[which(by_hc_lm_data$Factor==f)] <- p.adjust(by_hc_lm_data$pval[which(by_hc_lm_data$Factor==f)], method="fdr")
}

#We create a new field for the adjusted significance, which is same as original significant base (i.e. Positive/Negative so same first 8 letters)
by_hc_lm_data["sigadj"] <- substr(by_hc_lm_data$sig,1,8)

#Now we go through each adjusted p-value and coefficients to generate adjusted significance
for (r in 1:nrow(by_hc_lm_data)) {
  padj_r <- by_hc_lm_data$padj[r]
  #Now if the adjusted p-value is below 0.05 we add significcme
  if (padj_r<0.05) {
    by_hc_lm_data$sigadj[r] <- paste0(by_hc_lm_data$sigadj[r]," (Significant)")
  }
}

#Now we plot those results 
by_hc_lm_plot <- ggplot(by_hc_lm_data, aes(x = Factor, y = DisplayOutcome, color=sigadj)) +
...
###Data Prepration
#We will be pulling the names of the cells that changed on the diet
by_sig_ag <- paste0(by_patient_ag_lm_data$DisplayOutcome[which(by_patient_ag_lm_data$Factor=="Timepoint" & by_patient_ag_lm_data$padj<0.05)], "_Norm")

#Now we will select that data for the violin plots
select_by_all_proc_imp <- by_all_proc_imp[,c(clinvars, "CD8+CD40+_Norm")]

#Now we pivot to long format for the values 
select_by_long <- pivot_longer(select_by_all_proc_imp, cols="CD8+CD40+_Norm", names_to="Measure", values_to="Amount")

#Now we add the DisplayOutcome field 
select_by_long$DisplayOutcome <- gsub("_Norm","",select_by_long$Measure)

#Now we add the DiplayOutcomeFull Field 
select_by_long <- select_by_long %>%
  mutate(DisplayOutcomeFull = ifelse(
    substr(DisplayOutcome, 1, 3) %in% c("CD8", "CD4") & !grepl("T cell", DisplayOutcome, ignore.case = TRUE) & !grepl("Tregs", DisplayOutcome, ignore.case = TRUE),
    paste0(DisplayOutcome, " T Cells"),
    DisplayOutcome
...
#Below we make some aesthetic changes for plotting
#Now we remove % of parent
select_by_long$DisplayOutcomeFull <- gsub("\\(% of parent\\)","",select_by_long$DisplayOutcomeFull)
#We now correct the 16+ to be CD16+ 
select_by_long$DisplayOutcomeFull <- gsub("\\+16","+CD16",select_by_long$DisplayOutcomeFull)
##We now provide line dividers as well
select_by_long$DisplayOutcomeFull <- gsub("\\(%","\n\\(%",select_by_long$DisplayOutcomeFull)
select_by_long$DisplayOutcomeFull <- gsub("l Mono","l \nMono",select_by_long$DisplayOutcomeFull)
select_by_long$DisplayOutcomeFull <- gsub("e Mono","e \nMono",select_by_long$DisplayOutcomeFull)


###Statistics Preparation
#We add fields to the original statistics for ggplot to recognize

by_patient_ag_lm_data["Diet"] <- "Agrarian"
by_patient_ag_lm_data["group1"] <- "T1"
by_patient_ag_lm_data["group2"] <- "T3"

by_patient_we_lm_data["Diet"] <- "Western"
by_patient_we_lm_data["group1"] <- "T1"
by_patient_we_lm_data["group2"] <- "T3"

#We not stack the two on top of each other
by_all_stats <- rbind(by_patient_we_lm_data,by_patient_ag_lm_data)

#Now we focus on the stats of interest
by_all_sig_stats <- by_all_stats[which(paste0(by_all_stats$DisplayOutcome,"_Norm") %in% by_sig_ag & by_all_stats$Factor=="Timepoint"),]

by_all_sig_stats <- by_all_sig_stats %>%
  mutate(DisplayOutcomeFull = ifelse(
    substr(DisplayOutcome, 1, 3) %in% c("CD8", "CD4") & !grepl("T cell", DisplayOutcome, ignore.case = TRUE) & !grepl("Tregs", DisplayOutcome, ignore.case = TRUE),
    paste0(DisplayOutcome, " T Cells"),
    DisplayOutcome
...
#Below we maker aesthetic changes
#Now we remove % of parent
by_all_sig_stats$DisplayOutcomeFull <- gsub("\\(% of parent\\)","",by_all_sig_stats$DisplayOutcomeFull)
#We now correct the 16+ to be CD16+ 
by_all_sig_stats$DisplayOutcomeFull <- gsub("\\+16","+CD16",by_all_sig_stats$DisplayOutcomeFull)
##We now provide line dividers as well
by_all_sig_stats$DisplayOutcomeFull <- gsub("\\(%","\n\\(%",by_all_sig_stats$DisplayOutcomeFull)
by_all_sig_stats$DisplayOutcomeFull <- gsub("l Mono","l \nMono",by_all_sig_stats$DisplayOutcomeFull)
by_all_sig_stats$DisplayOutcomeFull <- gsub("e Mono","e \nMono",by_all_sig_stats$DisplayOutcomeFull)

#Now we add asterisks 
by_all_sig_stats$p.signif <- add_asterisks(by_all_sig_stats$padj)

by_select_viol <- ggplot(data = select_by_long,
                         mapping = aes(x = Timepoint, y = as.numeric(Amount), fill = Diet)) +
  geom_violin() +
  geom_line(mapping = aes(group = StudyID),
            position = position_dodge(0.1),
...
bd_select_viol <- ggplot(data = select_long,
                      mapping = aes(x = Timepoint, y = as.numeric(Amount), fill = Diet)) +
              geom_violin() +
              geom_line(mapping = aes(group = StudyID),
                position = position_dodge(0.1),
