...
$$Metabolic Outcome = Age + HIV + MSM + (1|StudyID)$$
```{r Basic Dietary LMEM, message=TRUE, warning=TRUE}

# build models and test for significance 
# First we do a new data set to change the names of metabolic outcome values
met_lmer_data <- long

#We go through the columns and remove the _mg-dL which causes errors
for (c in 29:39) {
  colnames(met_lmer_data)[c] <- gsub("_mg-dL","",colnames(met_lmer_data)[c])
}

#We rename out corresponding met_vars to reduce errors
lmer_met_vars <- gsub("_mg-dL","", met_vars)

#The basic dietary variables are defined by the column names 
basic_diet_vars <- grep("Total", colnames(long)[85:104], value=TRUE)
#We also add cohort for the linear mixed effect model 
basic_diet_vars <- c(basic_diet_vars, "Cohort")

#I set up a basic diet data set for the linear mixed effects results

#We generate an linear mixed effects model data set
basic_diet_lm_data <- unique(expand.grid(basic_diet_vars, lmer_met_vars))
colnames(basic_diet_lm_data) <- c("Factor","Outcome")
#We generate fields for the statistics
basic_diet_lm_data["pval"] <- NA
basic_diet_lm_data["coef"] <- NA
basic_diet_lm_data["sig"] <- NA

for (response in lmer_met_vars){ 
  # create full model 
  ff <- as.formula(paste0(response,"~ Total_Dairy + Total_Fruit + Total_Veg + Total_Protein + Total_Grain + Total_Fast_Food + Total_Drinks + Total_Snacks  + Cohort + (1|PID)")) 
  fm <- lmer(ff,
            data = met_lmer_data,
            REML = F)

  for(predictor in basic_diet_vars){ 
    # create null models 
    pred <- basic_diet_vars [! basic_diet_vars %in% c(predictor)]
    pred <- paste0(pred, collapse = "+")
    nf <- as.formula((paste0(response, "~", pred, "+(1|PID)")))
    nm <- lmer(nf, 
               data = met_lmer_data, 
               REML = F)
    ...
    res <- anova(nm, fm)
    betas <- fm@beta
    cols <- c("Intercept", basic_diet_vars)
    ...
    #We now identify the row in the stat table and add the corresponding p and beta values
      ...
      stat_row <- which(basic_diet_lm_data$Factor==predictor & basic_diet_lm_data$Outcome==response)
      basic_diet_lm_data$pval[stat_row] <- res$`Pr(>Chisq)`[2]
      basic_diet_lm_data$coef[stat_row] <- spec_beta
      ...
      basic_diet_lm_data$sig[stat_row] <- sig
...
basic_diet_plot <- ggplot(basic_diet_lm_data, aes(x = Factor, y = Outcome, color=sig)) +
