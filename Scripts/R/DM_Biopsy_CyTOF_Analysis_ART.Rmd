---
title: "DM Biopsy CyTOF ANalysis"
author: "J. O'Connor"
date: "2023-10-30"
...
#samp_inc <- samp_na$samp[which(as.numeric(samp_na$Count)<(length(by_cytdata_withmet[1,])/2))]
#col_inc <- col_na$col[which(as.numeric(col_na$Count)<(length(by_cytdata_withmet[,1])/2))]
samp_inc <- samp_na_by$samp[which(as.numeric(samp_na_by$Count)<50)]
...
#First Processed
by_all_proc_imp_for_biom <- by_all_proc_imp[,18:ncol(by_all_proc_imp)]
origcol_proc_imp <- origcol
by_all_proc_imp_biom <- make_biom(t(by_all_proc_imp_for_biom))
#Then Raw
by_all_raw_for_biom <- by_all_raw[,18:ncol(by_all_raw)]
origcol_raw <- colnames(by_all_raw_for_biom)
colnames(by_all_raw_for_biom) <- paste0("c",1:ncol(by_all_raw_for_biom))
by_all_raw_biom <-  make_biom(t(na.omit(by_all_raw_for_biom))) 
#We also export biom format data
write_biom(by_all_proc_imp_biom,"Data/Exported_from_R/by_all_proc.biom")
write_biom(by_all_raw_biom,"Data/Exported_from_R/by_all_raw.biom")

#We also need to store the column names as well

colnames_table <- cbind(paste0("c",1:ncol(by_all_raw_for_biom)), origcol_raw, origcol_proc_imp)
...
sig_ag_proc_cells <- ag_volc_data_tm_proc$var_names[which(ag_volc_data_tm_proc$`p-value`<0.05)] 
...
by_all_proc_imp <- inner_join(by_all_proc_imp,ART_data, by="StudyID")
ann_data <- by_all_proc_imp[,c(1:17,71:81)]

pcoa_data <- by_all_proc_imp[,18:66]
...
pcoaplot_biplot <- ggplot() + 
            ...
            labs(x=paste0("PCo1 (",round(imp_pcoa[2,1]*100, digits=1),"%)"), y=paste0("PCo2 (",round(imp_pcoa[2,2]*100, digits = 1),"%)"), key=" ", title="PCoA of Biopsy CyTOF Data")+
