...
Linear modelling is used to compare immune cell populations to patient data dn metabolic and immune markers

####Patient Data

$$Immune Cell = HIV + MSM + Diet + Timepoint + (1|StudyID)$$
...
patient_vars <- c("HIV","MSM","Diet","Timepoint")
...
for (response in cell_vars){ 
  # create full model 
  ff <- as.formula(paste0(response," ~ HIV + MSM + Diet + Timepoint + (1|StudyID)")) 
  fm <- lmer(ff,
            data = lmer_data,
            REML = F)
